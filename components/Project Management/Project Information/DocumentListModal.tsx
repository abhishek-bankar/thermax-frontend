...
const fieldObject: any = {
  ...
  local_isolator_specifications_and_list:
    "Local Isolator Specifications and List",
  ...
  earthing_calculation_input_sheet_will_be_provided:
    "Earthing Calculation (Input Sheet will be provided)",
...
const getFieldSchema = (panelIds: string[]) => {
  const staticFieldSchema = zod.object(
    Object.keys(fieldObject).reduce((acc, key) => {
  ...
  const dynamicPanelSchema = zod.object(
    panelIds?.reduce((acc, panelId) => {
      ...
      acc[`dynamic-${panelId}-panel_specification`] = zod
        .string()
        .optional() as any;
      acc[`dynamic-${panelId}-tentative_panel_gad`] = zod
        .string()
        .optional() as any;
      return acc;
...
const getDefaultValues = (staticValues: any, dynamicValues: any) => {
  const staticDefaultValues = Object.keys(fieldObject).reduce((acc, key) => {
    acc[key] = staticValues?.[key] || "TBD";
    return acc;
  }, {} as Record<string, string>);

  const dynamicDefaultValues = dynamicValues?.reduce((acc: any, panel: any) => {
    acc[`dynamic-${panel?.panel_id}-sld`] = panel?.sld || "TBD";
    acc[`dynamic-${panel?.panel_id}-panel_specification`] =
      panel?.panel_specification || "TBD";
    acc[`dynamic-${panel?.panel_id}-tentative_panel_gad`] =
      panel?.tentative_panel_gad || "TBD";
    return acc;
...
export default function DocumentListModal({
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const quotedPanelIds = panel_ids
        ?.map((id: string) => `"${id}"`)
        .join(", ");
  ...
  const handleStaticDocumentData = async (data: any) => {
    try {
      await updateData(
        `${STATIC_DOCUMENT_API}/${params.project_id}`,
        false,
        data
      );
    } catch (error: any) {
    ...
    >
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className="flex justify-between bg-black">
          ...
          <div className="flex-1 border p-1 text-center font-semibold text-white">
            Document Name
          </div>
        </div>
