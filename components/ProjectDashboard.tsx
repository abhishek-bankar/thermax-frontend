...
const ProjectStats = ({ projectData }: { projectData: any }) => {
  ...
  const data = [
    {
      ...
      values: years.map((year) => {return {count: getDisplayFY(year), year: year}}),
    },
    {
      label: "Total Projects",
      // values: years.map((year) => stats[year].total),
      values: years.map((year) => {return {count: stats[year].total, year: year}}),
    },
    {
      label: "Live Projects",
      values: years.map((year) => {return {count: stats[year].live, year: year}}),

      // values: years.map((year) => stats[year].live),
    },
    {
      label: "Completed Projects",
      values: years.map((year) => {return {count: stats[year].completed, year: year}}),
  ...
  return (
    <div className="w-1/2 mx-auto p-4">
      <div className="overflow-x-auto rounded backdrop-blur-lg bg-white/10">
        <table className="min-w-full border-separate border-spacing-x-4 p-4">
          <tbody>
...
const ProjectDashboard = () => {
  ...
  const { data: projectList } = useGetData(getProjectUrl);
